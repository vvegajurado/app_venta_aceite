...
// Se ha corregido esta línea para incluir 'get_direcciones_envio_ajax' en las acciones GET.
...
if ($is_ajax_request) {
        ...
        if (isset($_POST['accion'])) {
            switch ($_POST['accion']) {
                ...
                case 'agregar_direccion_envio':
                    ...
                    // Actualizar la sentencia INSERT para incluir los nuevos campos
                ...
                case 'editar_direccion_envio':
                    ...
                    $codigo_postal = trim($_POST['codigo_postal_envio'] ?? '');
                    $latitud = filter_var($_POST['latitud_envio'] ?? '', FILTER_VALIDATE_FLOAT) !== false ? (float)$_POST['latitud_envio'] : null;
                    $longitud = filter_var($_POST['longitud_envio'] ?? '', FILTER_VALIDATE_FLOAT) !== false ? (float)$_POST['longitud_envio'] : null; // Changed from $_POST['longitud'] to $_POST['longitud_envio']
                    $direccion_google_maps = trim($_POST['direccion_google_maps_envio'] ?? '');
                    $es_principal = isset($_POST['es_principal']) ? 1 : 0;
                    // Nuevos campos
                    $telefono_envio = trim($_POST['telefono_envio'] ?? '');
                    ...
                    // Actualizar la sentencia UPDATE para incluir los nuevos campos
                ...
                case 'agregar_ajax': // Usado para añadir cliente desde el modal de pedidos
                    ...
                    $nif = trim($_POST['nif'] ?? ''); // NIF puede ser vacío, se guardará como NULL
                    ...
                    $observaciones = trim($_POST['observaciones'] ?? ''); // Nuevo campo

                    if (empty($nombre_cliente)) {
                        throw new Exception("Error: El nombre del cliente es obligatorio.");
                    }

                    // Convertir NIF vacío a NULL para la base de datos
                    ...
                    $stmt->execute([$nombre_cliente, $direccion, $ciudad, $provincia, $codigo_postal, $telefono, $email, $nif_db, $latitud, $longitud, $direccion_google_maps, $observaciones]); // Añadir $observaciones
                    $new_id = $pdo->lastInsertId();
                    ...
                    echo json_encode(['success' => true, 'message' => 'Cliente agregado correctamente.', 'cliente' => [
                        ...
                        'observaciones' => $observaciones // Añadir observaciones a la respuesta AJAX
                    ]]);
                    break;
            }
            ...
            switch ($_GET['accion']) {
                case 'buscar_ajax':
                    ...
                    if (!empty($search_query)) {
                        $where_clauses[] = "(c.nombre_cliente LIKE ? OR c.nif LIKE ? OR c.ciudad LIKE ? OR c.provincia LIKE ? OR c.telefono LIKE ? OR c.email LIKE ? OR c.observaciones LIKE ?)"; // Añadir observaciones a la búsqueda
                        $like_param = '%' . $search_query . '%';
                        $params = [$like_param, $like_param, $like_param, $like_param, $like_param, $like_param, $like_param]; // Añadir un parámetro más
                    }
                ...
                case 'get_direcciones_envio_ajax':
                    ...
                    // Seleccionar también los nuevos campos
    ...
    exit(); // Salir inmediatamente después de enviar JSON para peticiones AJAX
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre_cliente)) {
        ...
        try {
            if ($id_cliente_form) {
                // Modo EDICIÓN
                // Añadir 'observaciones = ?' a la sentencia UPDATE
                ...
                $stmt->execute([$nombre_cliente, $direccion, $ciudad, $provincia, $codigo_postal, $telefono, $email, $nif_db, $latitud, $longitud, $direccion_google_maps, $observaciones, $id_cliente_form]); // Añadir $observaciones
                $mensaje = "Cliente actualizado correctamente.";
                $tipo_mensaje = 'success';
            } else {
                // Modo AGREGAR
                // Añadir 'observaciones' a la sentencia INSERT
                $stmt = $pdo->prepare("INSERT INTO clientes (nombre_cliente, direccion, ciudad, provincia, codigo_postal, telefono, email, nif, latitud, longitud, direccion_google_maps, observaciones) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$nombre_cliente, $direccion, $ciudad, $provincia, $codigo_postal, $telefono, $email, $nif_db, $latitud, $longitud, $direccion_google_maps, $observaciones]); // Añadir $observaciones
...
// --- Lógica para cargar un cliente específico si se pasa un ID por GET (para pre-rellenar el formulario) ---
$id_cliente_param = null;
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $id_cliente_param = $_GET['id'];
    try {
        // Asegurarse de seleccionar la columna 'observaciones'
...
if ($id_cliente_param) {
    ...
    // Añadir c.observaciones a la cláusula WHERE
...
try {
    ...
    foreach ($clientes as &$cliente) { // Usar & para modificar el array original
        $cliente['total_deuda_cliente'] = bcsub((string)$cliente['total_facturado_cliente'], (string)$cliente['total_cobrado_cliente'], 2);
    }
    unset($cliente); // Romper la referencia al último elemento
} catch (PDOException $e) {
...
<head>
    ...
    <style>
        ...
        .main-content {
            ...
            padding: 30px;
            background-color: #f4f7f6;
            /* Eliminado width y margin-left para que flexbox lo gestione */
            min-height: 100vh; /* Asegura que el contenido principal también ocupe toda la altura */
        }
        .container-fluid { /* Cambiado de .container a .container-fluid para ocupar más ancho */
...
<body>
    ...
    <div class="main-content">
        <div class="container-fluid">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="input-group mb-3">
                        ...
                        <?php if (!empty($search_query) || $id_cliente_param): // Mostrar botón de limpiar si hay búsqueda o ID en URL ?>
                            <a href="clientes.php" class="btn btn-outline-danger"><i class="bi bi-x-circle"></i> Limpiar Filtros</a>
                        <?php endif; ?>
                    </div>

                    <div class="table-responsive">
                        <div style="max-height: 250px; overflow-y: auto; border: 1px solid #ccc; margin-bottom: 1em;"><table class="table table-striped table-hover">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Nombre</th>
                                    ...
                                    <th>Provincia</th>
                                    <th>Teléfono</th>
                                    <!-- Columnas Email, Latitud, Longitud eliminadas -->
                            ...
                            <tbody id="clients_table_body">
                                <?php if (empty($clientes)): ?>
                                    <tr>
                                        <td colspan="9" class="text-center">No hay clientes registrados o no se encontraron resultados.</td> <!-- Colspan ajustado a 9 -->
                                    </tr>
                                <?php else: ?>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <!-- Datos de Email, Latitud, Longitud eliminados -->
                                            <td><?php echo htmlspecialchars($cliente['direccion_google_maps'] ?? 'N/A'); ?></td>
                                            <td><?php echo htmlspecialchars($cliente['observaciones'] ?? 'N/A'); ?></td> <!-- Mostrar observaciones -->
            ...
            <!-- Nueva Sección de Direcciones de Envío (Visible solo al editar un cliente) -->
            <div class="card mb-4" id="client_shipping_addresses_card" style="display:none;">
                <div class="card-header">
                    Direcciones de Envío
                </div>
                <div class="card-body">
                    <button type="button" class="btn btn-success mb-3" onclick="openAddEditDireccionEnvioModal(null)">
                        <i class="bi bi-plus-circle"></i> Añadir Nueva Dirección de Envío
                    </button>
                    <div class="table-responsive">
                        <div style="max-height: 250px; overflow-y: auto; border: 1px solid #ccc; margin-bottom: 1em;"><table class="table table-striped table-hover">
                            <thead>
    ...
    <script>
        ...
        // geocoderEnvio is not needed, as `geocoder` can be reused.
        ...
        window.initMapPicker = function() {
            ...
            geocoder = new google.maps.Geocoder(); // Inicializar el geocodificador una vez
        ...
        function initMapEnvio() {
            ...
            const initialLatEnvio = parseFloat(latInputEnvio.value) || 37.3828300; // Default a Sevilla
        ...
        function geocodeAddressEnvio() {
            ...
            if (!geocoder) { // Reusing the same geocoder instance
                showCustomAlert('El servicio de mapas no está listo. Inténtelo de nuevo en unos segundos.');
                return;
            }
        ...
        async function editClient(cliente) {
            ...
            document.getElementById('observaciones').value = cliente.observaciones || ''; // Rellenar observaciones
        ...
        function getOrderStatusBadgeClass(status) {
            switch (status) {
                ...
                case 'parcialmente_facturado': return 'bg-primary'; // Added for consistency
        ...
        function resetForm() {
            ...
            document.getElementById('observaciones').value = ''; // Limpiar observaciones
            document.getElementById('latitud').value = '';
            document.getElementById('longitud').value = '';
            document.getElementById('direccion_google_maps').value = '';
        ...
        // NUEVA FUNCIÓN: Para redirigir a pedidos.php con el ID y nombre del cliente y abrir el modal
        ...
        function renderClientsTable(clients) {
            let html = '';
            if (clients.length === 0) {
                html = `<tr><td colspan="9" class="text-center">No hay clientes registrados o no se encontraron resultados.</td></tr>`; // Colspan ajustado a 9
            } else {
                clients.forEach(cliente => {
                    ...
                    html += `
                        <tr>
                            ...
                            <td>${cliente.observaciones || 'N/A'}</td> <!-- Mostrar observaciones -->
        ...
        async function performSearch() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    clientsTableBody.innerHTML = `<tr><td colspan="9" class="text-center text-danger">Error al cargar los clientes: ${data.message}</td></tr>`; // Colspan ajustado a 9
                }
            } catch (error) {
                console.error('Error en la petición de búsqueda:', error);
                clientsTableBody.innerHTML = `<tr><td colspan="9" class="text-center text-danger">Error de red o servidor al buscar clientes.</td></tr>`; // Colspan ajustado a 9
            }
        }
        ...
        let currentEditingDireccionEnvio = null; // Para saber si estamos añadiendo o editando

        async function loadShippingAddresses(clientId) {
            const shippingAddressesList = document.getElementById('shipping_addresses_list');
            shippingAddressesList.innerHTML = '<tr><td colspan="6" class="text-center text-muted">Cargando direcciones de envío...</td></tr>'; // Changed colspan to 6
            try {
                ...
                if (data.success && data.direcciones.length > 0) {
                    ...
                    data.direcciones.forEach(dir => {
                        ...
                        html += `
                            <tr>
                                ...
                                <td>${dir.direccion}, ${dir.ciudad || ''}, ${dir.provincia || ''} ${dir.codigo_postal || ''} (${dir.direccion_google_maps || 'N/A'})</td>
                    ...
                    shippingAddressesList.innerHTML = '<tr><td colspan="6" class="text-center text-muted">No hay direcciones de envío registradas para este cliente.</td></tr>'; // Changed colspan to 6
                }
            } catch (error) {
                console.error('Error loading shipping addresses:', error);
                shippingAddressesList.innerHTML = '<tr><td colspan="6" class="text-center text-danger">Error al cargar direcciones de envío.</td></tr>'; // Changed colspan to 6
            }
        }
        ...
        function openAddEditDireccionEnvioModal(direccion = null) {
            ...
            document.getElementById('id_cliente_direccion_envio').value = document.getElementById('id_cliente_form').value; // Cliente actual
        ...
        async function submitDireccionEnvioForm(event) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    loadShippingAddresses(currentClientId); // Recargar la lista de direcciones
                } else {
        ...
        function confirmDeleteDireccionEnvio(idDireccion, idCliente) {
            showCustomAlert("¿Estás seguro de que quieres eliminar esta dirección de envío?", 'confirm', async (confirmed) => { // Changed message to be a direct question
                if (confirmed) {
                    const formData = new FormData();
                    ...
                    try {
                        ...
                        if (data.success) {
                            showCustomAlert(data.message, 'success');
                            loadShippingAddresses(idCliente); // Recargar la lista de direcciones
                        } else {
                            showCustomAlert(data.message, 'danger');
                        }
                    } catch (error) {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (clientIdFromUrl) {
                ...
                <?php endif; ?>
                // No need to scroll here, editClient already scrolls to 0,0
            }
        });
    </script>
