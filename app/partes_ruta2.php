...
// --- INTENTO DE AÑADIR RESTRICCIÓN UNIQUE EN LA BASE DE DATOS PARA EVITAR DUPLICADOS ---
try {
    // Comprobar si la restricción ya existe
    $stmt = $pdo->query("SHOW INDEX FROM partes_ruta_pedidos WHERE Key_name = 'unique_parte_pedido'");
    $indexExists = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$indexExists) {
        // Si no existe, intentar añadirla
        $pdo->exec("ALTER TABLE partes_ruta_pedidos ADD CONSTRAINT unique_parte_pedido UNIQUE (id_parte_ruta, id_pedido)");
        error_log("Restricción UNIQUE 'unique_parte_pedido' añadida a la tabla 'partes_ruta_pedidos'.");
    }
} catch (PDOException $e) {
    // Si hay un error (ej. duplicados existentes), registrarlo pero no detener la ejecución.
    // El usuario deberá limpiar los duplicados manualmente si este error ocurre.
    error_log("Error al añadir la restricción UNIQUE a partes_ruta_pedidos: " . $e->getMessage());
    // Puedes añadir un mensaje al usuario si quieres que sea visible
    // $mensaje = "Advertencia: No se pudo aplicar la restricción de unicidad en la base de datos debido a duplicados existentes. Por favor, revise los logs del servidor o su base de datos.";
    // $tipo_mensaje = 'warning';
}
// --- FIN DEL BLOQUE DE RESTRICCIÓN UNIQUE ---
    ...
    try {
        ...
        $sql = "
            SELECT
                ...
                p.total_pedido_iva_incluido AS total_pedido, -- Usar el campo total_pedido_iva_incluido
            ...
            LEFT JOIN
                detalle_factura_ventas dfv ON fv.id_factura = dfv.id_factura AND dp.id_producto = dfv.id_producto -- Join for factured/served quantities
            WHERE
        ...
        $sql .= "
            GROUP BY
                p.id_pedido, p.fecha_pedido, p.id_cliente, c.nombre_cliente, p.id_factura_asociada, fv.estado_pago, p.total_pedido_iva_incluido
            HAVING
                (total_unidades_servidas < total_cantidad_facturada OR p.id_factura_asociada IS NULL) -- No completamente servido o sin factura
            ORDER BY
                p.fecha_pedido ASC;
        ";
    ...
    try {
        $stmt = $pdo->prepare("
            ...
            WHERE
                prp.id_parte_ruta = ?
            GROUP BY
                p.id_pedido, p.fecha_pedido, p.id_cliente, c.nombre_cliente, p.total_pedido_iva_incluido,
                p.estado_pedido, p.id_ruta, c.latitud, c.longitud, c.direccion_google_maps, p.tipo_entrega,
                p.empresa_transporte, p.numero_seguimiento, p.observaciones, p.id_direccion_envio,
                de.latitud, de.longitud, de.direccion_google_maps, de.nombre_direccion
            ORDER BY
                p.fecha_pedido ASC
        ");
...
if (isset($_GET['action']) && $_GET['action'] === 'get_pedidos_details_by_parte_ruta') {
    ...
    try {
        // MODIFICACIÓN CLAVE AQUÍ: Agrupar por p.id_pedido para evitar duplicados en la visualización
        // Se selecciona MIN(prp.id_parte_ruta_pedido) y MIN(prp.observaciones_detalle)
        // para asegurar que cada pedido aparezca una sola vez.
        $stmt_pedidos = $pdo->prepare("
            SELECT
                ...
                p.id_factura_asociada,
                fv.estado_pago,
                MIN(prp.id_parte_ruta_pedido) AS id_parte_ruta_pedido, -- Tomar el MIN ID para identificar la fila única en partes_ruta_pedidos
                MIN(prp.observaciones_detalle) AS observaciones_detalle, -- Tomar la primera observación si hay múltiples
                c.latitud,
                c.longitud,
                c.direccion_google_maps,
                p.tipo_entrega,
            ...
            WHERE
                prp.id_parte_ruta = ?
            GROUP BY
                p.id_pedido, p.fecha_pedido, p.id_cliente, c.nombre_cliente,
                p.total_pedido_iva_incluido, p.estado_pedido, p.id_factura_asociada, fv.estado_pago,
                c.latitud, c.longitud, c.direccion_google_maps, p.tipo_entrega,
                p.empresa_transporte, p.numero_seguimiento, p.observaciones,
                p.id_direccion_envio, de.latitud, de.longitud, de.direccion_google_maps, de.nombre_direccion
            ORDER BY
                p.fecha_pedido ASC
        ");
        $stmt_pedidos->execute([$id_parte_ruta]);
        $pedidos_data_raw = $stmt_pedidos->fetchAll(PDO::FETCH_ASSOC);

        // PHP-side de-duplication for AJAX response
        $pedidos_data = [];
        $seen_pedido_ids = []; // Changed to array for in_array check
        foreach ($pedidos_data_raw as $pedido) {
            if (!in_array($pedido['id_pedido'], $seen_pedido_ids)) {
                $pedidos_data[] = $pedido;
                $seen_pedido_ids[] = $pedido['id_pedido'];
            }
        }


        foreach ($pedidos_data as &$pedido) {
            // Fetch total_unidades for the current pedido
            $stmt_total_unidades = $pdo->prepare("
                SELECT COALESCE(SUM(dp.cantidad_solicitada), 0) AS total_unidades_sum
                FROM detalle_pedidos dp
                WHERE dp.id_pedido = ?
            ");
            $stmt_total_unidades->execute([$pedido['id_pedido']]);
            $pedido_total_unidades_result = $stmt_total_unidades->fetch(PDO::FETCH_ASSOC);
            $pedido['total_unidades'] = $pedido_total_unidades_result['total_unidades_sum'];
            ...
            $pedido_detalles_lineas = $stmt_detalles->fetchAll(PDO::FETCH_ASSOC);

            $can_serve_order_in_totality = true;
            foreach ($pedido_detalles_lineas as &$detalle_linea) {
                // Check if stock is sufficient for this line item
                $detalle_linea['can_serve_line_in_totality'] = ($detalle_linea['stock_actual_unidades'] >= $detalle_linea['cantidad_solicitada']);
                if (!$detalle_linea['can_serve_line_in_totality']) {
                    $can_serve_order_in_totality = false; // If any line cannot be served, the whole order cannot
                }
            }
            $pedido['detalles'] = $pedido_detalles_lineas;
            $pedido['can_serve_order_in_totality'] = $can_serve_order_in_totality; // Add this flag to the order
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['action'])) { // Check for general form submission, not specific action
    ...
    error_log("Recibido POST para parte de ruta. ID Parte: " . ($id_parte_ruta ?: 'Nuevo') . ", Pedidos (decoded): " . print_r($pedidos_seleccionados, true));
            ...
            if (!empty($pedidos_seleccionados)) {
                ...
                foreach ($pedidos_seleccionados as $pedido_id) {
                    try {
                        ...
                        // Si la restricción UNIQUE está activa y se intenta insertar un duplicado
                        if ($e->getCode() == 23000) { // SQLSTATE for Integrity Constraint Violation
                            error_log("Advertencia: Intento de añadir pedido duplicado " . $pedido_id . " al parte " . $id_parte_ruta . ". Ignorado debido a restricción UNIQUE.");
                            // Podrías añadir un mensaje al usuario aquí si lo consideras necesario
                        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_pedido_ruta_status') {
    ...
    if (empty($id_parte_ruta_pedido) || empty($new_status)) {
        echo json_encode(['success' => false, 'message' => 'ID de detalle de parte de ruta y nuevo estado son obligatorios.']);
        exit();
    }
    ...
    try {
        $stmt = $pdo->prepare("UPDATE partes_ruta_pedidos SET estado_pedido_ruta = ?, observaciones_detalle = ? WHERE id_parte_ruta_pedido = ?");
        $stmt->execute([$new_status, $new_observations, $id_parte_ruta_pedido]);

        if ($stmt->rowCount() > 0) {
            echo json_encode(['success' => true, 'message' => 'Estado del pedido en ruta actualizado exitosamente.']);
        } else {
...
try {
    ...
    $stmt_partes_ruta = $pdo->query("
        SELECT
            ...
            COALESCE(SUM(pedido_agg.total_litros_pedido), 0) AS total_litros_ruta,
            COALESCE(SUM(pedido_agg.total_euros_pedido), 0) AS total_euros_ruta,
            COALESCE(SUM(CASE WHEN fv.estado_pago = 'pagada' THEN fv.total_factura_iva_incluido ELSE 0 END), 0) AS total_cobrado,
            COALESCE(SUM(pedido_agg.total_euros_pedido), 0) - COALESCE(SUM(CASE WHEN fv.estado_pago = 'pagada' THEN fv.total_factura_iva_incluido ELSE 0 END), 0) AS total_deuda
        FROM
            partes_ruta pr
        JOIN
        ...
        LEFT JOIN (
            SELECT
                p.id_pedido,
                p.total_pedido_iva_incluido AS total_euros_pedido,
                SUM(dp.cantidad_solicitada) AS total_litros_pedido,
                p.id_factura_asociada
            FROM
                pedidos p
            LEFT JOIN
                detalle_pedidos dp ON p.id_pedido = dp.id_pedido
            GROUP BY
                p.id_pedido, p.total_pedido_iva_incluido, p.id_factura_asociada
        ) AS pedido_agg ON prp.id_pedido = pedido_agg.id_pedido
        LEFT JOIN
            facturas_ventas fv ON pedido_agg.id_factura_asociada = fv.id_factura
        GROUP BY
    ...
    foreach ($partes_ruta_existentes as $parte) {
        $totalLitrosGeneral += (float)$parte['total_litros_ruta'];
        $totalEurosGeneral += (float)$parte['total_euros_ruta'];
        $totalCobradoGeneral += (float)$parte['total_cobrado']; // Accumulate new total
        $totalDeudaGeneral += (float)$parte['total_deuda'];     // Accumulate new total
    }
...
if (isset($_GET['view_id']) && is_numeric($_GET['view_id'])) {
    ...
    try {
        ...
        p.id_factura_asociada,
        fv.estado_pago,
        MIN(prp.id_parte_ruta_pedido) AS id_parte_ruta_pedido,
        MIN(prp.observaciones_detalle) AS observaciones_detalle,
        MIN(prp.estado_pedido_ruta) AS estado_pedido_ruta,
        c.latitud,
        c.longitud,
        c.direccion_google_maps,
        p.tipo_entrega,
    ...
    ORDER BY
        p.fecha_pedido ASC
            ...
            $parte_ruta_pedidos_details_raw = $stmt_details->fetchAll(PDO::FETCH_ASSOC);

            // NEW: PHP-side de-duplication to ensure only one entry per id_pedido for display
            $parte_ruta_pedidos_details = [];
            $seen_pedido_ids_for_display = [];
            foreach ($parte_ruta_pedidos_details_raw as $pedido) {
                if (!in_array($pedido['id_pedido'], $seen_pedido_ids_for_display)) {
                    $parte_ruta_pedidos_details[] = $pedido;
                    $seen_pedido_ids_for_display[] = $pedido['id_pedido'];
                }
            }


            // Re-fetch details to include stock information for initial display
            foreach ($parte_ruta_pedidos_details as &$pedido) {
                ...
                $stmt_detalles_stock = $pdo->prepare("
                    SELECT
                        dp.id_detalle_pedido,
                        dp.id_producto,
                        prod.nombre_producto,
                    ...
                    WHERE
                        dp.id_pedido = ?
                ");
                $stmt_detalles_stock->execute([$pedido['id_factura_asociada'], $pedido['id_pedido']]);
                $pedido_detalles_lineas = $stmt_detalles_stock->fetchAll(PDO::FETCH_ASSOC);

                $can_serve_order_in_totality = true;
                foreach ($pedido_detalles_lineas as &$detalle_linea) {
                    $detalle_linea['can_serve_line_in_totality'] = ($detalle_linea['stock_actual_unidades'] >= $detalle_linea['cantidad_solicitada']);
                    if (!$detalle_linea['can_serve_line_in_totality']) {
                        $can_serve_order_in_totality = false;
                    }
                }
                $pedido['detalles'] = $pedido_detalles_lineas;
                $pedido['can_serve_order_in_totality'] = $can_serve_order_in_totality;
            }
            ...
            foreach ($parte_ruta_pedidos_details as $pedido) {
                $initial_selected_orders_for_js[$pedido['id_pedido']] = [
                    ...
                    'total_pedido' => $pedido['total_pedido'] ?? 0,
                    'id_factura_asociada' => $pedido['id_factura_asociada'],
                    'estado_pago' => $pedido['estado_pago'],
                    // These two fields are not directly selected in the main query after grouping by p.id_pedido
                    // We need to fetch them if they are truly needed for the initial_selected_orders_for_js
                    // For now, setting them to 0 or null as a placeholder, or removing if not critical for JS logic
                    'total_cantidad_facturada' => 0, // Placeholder, needs separate query if needed
                    'total_unidades_servidas' => 0,   // Placeholder, needs separate query if needed
                    ...
                    'nombre_direccion_envio' => $pedido['nombre_direccion_envio'] ?? null,
                    'latitud' => $pedido['latitud'],
                    'longitud' => $pedido['longitud'],
                    'direccion_google_maps' => $pedido['direccion_google_maps'],
                    'can_serve_order_in_totality' => $pedido['can_serve_order_in_totality'] ?? true
                ];
            }
                            ...
                            <table class="table table-bordered table-striped align-middle">
                                <thead>
                                    <tr>
                                        <th>ID Pedido</th>
                                        <th>Fecha Pedido</th>
                                        <th>Cliente / Dirección de Envío</th>
                                        <th class="text-end">Subtotal</th> <!-- Changed from Total Unidades -->
                                        <th>Observaciones Detalle</th>
                                        <th>Estado Factura</th>
                                        <th>Estado Cobro</th>
                                        <th>Estado Stock</th> <!-- NEW COLUMN HEADER -->
                                        <th class="text-center">Acciones</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php if (empty($parte_ruta_pedidos_details)): ?>
                                        <tr><td colspan="9" class="text-center">No hay pedidos asociados a este parte de ruta.</td></tr> <!-- Adjusted colspan -->
                                    <?php else: ?>
                                        <?php foreach ($parte_ruta_pedidos_details as $p):
                                            ...
                                            $displayClientInfo = htmlspecialchars($p['nombre_cliente']);
                                            if (!empty($p['id_direccion_envio'])) {
                                                // Check if nombre_direccion_envio exists and is not empty
                                                if (!empty($p['nombre_direccion_envio'])) {
                                                    $displayClientInfo .= '<br><small class="text-muted">(' . htmlspecialchars($p['nombre_direccion_envio']) . ')</small>';
                                                }
                                                // Always show Google Maps address if shipping address is used and available
                                                if (!empty($p['direccion_google_maps_envio'])) {
                                                    $displayClientInfo .= '<br><small class="text-muted">' . htmlspecialchars($p['direccion_google_maps_envio']) . '</small>';
                                                }
                                            }

                                            // Determine stock status for the order
                                            $stockStatusBadgeClass = 'bg-success';
                                            $stockStatusText = 'Stock Suficiente';
                                            if (isset($p['can_serve_order_in_totality']) && !$p['can_serve_order_in_totality']) {
                                                $stockStatusBadgeClass = 'bg-danger';
                                                $stockStatusText = 'Stock Insuficiente';
                                            }
                                        ?>
                                            <tr id="pedido-row-<?php echo htmlspecialchars($p['id_parte_ruta_pedido']); ?>">
                                                ...
                                                <td><?php echo $displayClientInfo; ?></td>
                                                <td class="text-end"><?php echo number_format($p['total_pedido'] ?? 0, 2, ',', '.'); ?>€</td> <!-- Displaying total_pedido -->
                                                <td>
                                                ...
                                                <td><span class="badge <?php echo $stockStatusBadgeClass; ?>"><?php echo $stockStatusText; ?></span></td> <!-- NEW CELL -->
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($partes_ruta_existentes as $parte): ?>
                                            <tr data-estado="<?php echo htmlspecialchars($parte['estado']); ?>"
                                                ...
                                                <td class="text-end">
                                                    <span class="<?php echo ((float)($parte['total_deuda'] ?? 0) > 0) ? 'text-danger' : ''; ?>">
    ...
    <script>
        ...
        async function calculateAndDisplayRoute(pedidos, optimize = true) {
            ...
            const validPedidos = pedidos.filter(pedido => {
                ...
                let addressDisplayName = pedido.nombre_cliente; // Default to client name
                if (pedido.id_direccion_envio && !isNaN(lat) && !isNaN(lng)) {
                    // Use shipping address coordinates and name
                    pedido._effective_lat = lat;
                    pedido._effective_lng = lng;
                    addressDisplayName = pedido.nombre_cliente;
                    if (pedido.nombre_direccion_envio) {
                        addressDisplayName += ` (${pedido.nombre_direccion_envio})`;
                    }
                    if (pedido.direccion_google_maps_envio) {
                        addressDisplayName += ` - ${pedido.direccion_google_maps_envio}`;
                    }
                    pedido._effective_address_name = addressDisplayName;
                    return true;
                } else {
                    // Fallback to client's main address coordinates
                    ...
                    if (!isNaN(clientLat) && !isNaN(clientLng)) {
                        ...
                        pedido._effective_address_name = `Dirección Principal Cliente: ${pedido.direccion_google_maps || pedido.direccion || 'N/A'}`;
                        return true;
                    }
                }
            ...
            currentRouteWaypoints = validPedidos.map(pedido => {
                ...
                const infoContent = `
                    <div class="info-window-content">
                        ...
                        ${pedido.nombre_direccion_envio ? `<p>Dirección Envío: ${pedido.nombre_direccion_envio}</p>` : ''}
            ...
            try {
                const request = {
                    ...
                    travelMode: google.maps.TravelMode.DRaged
                };
        ...
        async function loadPedidosDetailsForMap(idParteRuta) {
            ...
            try {
                ...
                if (data.success) {
                    // JavaScript-side de-duplication for the modal display
                    const uniquePedidos = [];
                    const seenPedidoIds = new Set();
                    data.pedidos.forEach(pedido => { // Use data.pedidos here
                        if (!seenPedidoIds.has(pedido.id_pedido)) {
                            uniquePedidos.push(pedido);
                            seenPedidoIds.add(pedido.id_pedido);
                        }
                    });

                    const orderIdToDetailsMap = new Map();
                    uniquePedidos.forEach(pedido => { // Use uniquePedidos here
                    ...
                    // Ahora iteramos sobre currentRouteWaypoints para mantener el orden optimizado
                    if (currentRouteWaypoints.length === 0) {
                        ...
                        currentRouteWaypoints.forEach(waypoint => {
                            ...
                            if (pedido) {
                                // Determine stock status for the order
                                const canServeOrder = pedido.can_serve_order_in_totality;
                                const stockStatusBadgeClass = canServeOrder ? 'bg-success' : 'bg-danger';
                                const stockStatusText = canServeOrder ? 'Stock Suficiente' : 'Stock Insuficiente';
                                ...
                                htmlContent += `
                                    <div class="order-item-card">
                                        ...
                                        <p><strong>Estado de Stock:</strong> <span class="badge ${stockStatusBadgeClass}">${stockStatusText}</span></p> <!-- NEW: Stock Status -->
                                            ...
                                            <table class="table table-bordered table-sm">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th>Stock Actual</th> <!-- NEW: Stock Actual -->
                                ...
                                if (pedido.detalles && pedido.detalles.length > 0) {
                                    pedido.detalles.forEach(detalle => {
                                        const lineStockStatusBadgeClass = detalle.can_serve_line_in_totality ? 'bg-success' : 'bg-danger';
                                        const lineStockStatusText = detalle.can_serve_line_in_totality ? 'Suficiente' : 'Insuficiente';

                                        htmlContent += `
                                            <tr>
                                                <td>${detalle.nombre_producto}</td>
                                                <td>${detalle.cantidad_solicitada}</td>
                                                <td><span class="badge ${lineStockStatusBadgeClass}">${detalle.stock_actual_unidades} (${lineStockStatusText})</span></td> <!-- CORREGIDO: Usar stock_actual_unidades -->
                                    ...
                                    htmlContent += `<tr><td colspan="6" class="text-center">No hay líneas de detalle para este pedido.</td></tr>`; // Adjusted colspan
                                }
                                htmlContent += `
                                                </tbody>
                                            </table>
        ...
        function generateParteRutaPdf(idParteRuta) {
            // Obtener la lista de IDs de pedidos en el orden optimizado
            const orderedPedidoIds = currentRouteWaypoints.map(wp => wp.idPedido).filter(id => id !== undefined && id !== null);

            // Convertir el array de IDs a una cadena JSON para pasarla por la URL
            const orderedPedidosJson = JSON.stringify(orderedPedidoIds);

            // Construir la URL con el ID del parte de ruta y la lista de pedidos ordenada
            const pdfUrl = `generate_parte_ruta_pdf.php?id_parte_ruta=${idParteRuta}&ordered_pedidos=${encodeURIComponent(orderedPedidosJson)}`;
            
            console.log("DEBUG: URL para PDF:", pdfUrl); // Log para depuración
        ...
        function updateSelectedOrdersDisplay(container, pedidosInput) { // Removed noOrdersMessage from arguments
            ...
            if (selectedOrders.size === 0) {
                        ...
                        <table class="table table-bordered table-striped align-middle">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Estado Stock</th> <!-- NEW: Stock Status Header -->
                                    <th class="text-center">Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                ...
                selectedOrders.forEach((order, id) => {
                    ...
                    const totalPedido = order.total_pedido ?? 0; // Changed from total_importe
                    ...
                    // Determine stock status for the order
                    const canServeOrder = order.can_serve_order_in_totality;
                    const stockStatusBadgeClass = canServeOrder ? 'bg-success' : 'bg-danger';
                    const stockStatusText = canServeOrder ? 'Suficiente' : 'Insuficiente';


                    // Determine which address to display in the modal's selected orders list
                    let displayClientInfoInModal = order.nombre_cliente;
                    if (order.id_direccion_envio && order.nombre_direccion_envio) {
                        displayClientInfoInModal += ` (${order.nombre_direccion_envio})`;
                    }
                    if (order.id_direccion_envio && order.direccion_google_maps_envio) {
                        displayClientInfoInModal += `<br><small class="text-muted">${order.direccion_google_maps_envio}</small>`;
                    }
                    ...
                    tableHtml += `
                        <tr>
                            ...
                            <td>${displayClientInfoInModal}</td>
                            <td class="text-end">${parseFloat(totalPedido).toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€</td>
                            <td class="text-center">
                            ...
                            <td><span class="badge ${stockStatusBadgeClass}">${stockStatusText}</span></td> <!-- NEW: Stock Status Cell -->
                            <td class="text-center">
        ...
        function updateFooterTotals() {
            ...
            document.getElementById('footerTotalLitros').textContent = number_format(totalLitros, 2, ',', '.') + ' uds.'; // Added units
                            ...
                            <div class="form-check d-flex align-items-center w-100">
                                ...
                                <div>
                                    ...
                                    <div class="d-flex mt-1">
                                        ...
                                        <span class="badge bg-warning text-dark"><i class="bi bi-currency-euro"></i> Importe: ${parseFloat(totalPedido).toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€</span>
